...
This recommendation also mitigate a type of attacks that flood the local network
with mDNS messages, as detailed in Section {{#security}}.
...
mDNS messages are generated and multicast in the process of name registration,
resolution and removal. In particular,
...
- creating browsing contexts that create an ICE agent and start gathering of
candidates and
- destroying them soon after the name registration is done.
...
transmission. This rate limiting rule however does not mitigate the above attack,
in which new names, hence new records, are constantly created and sent. A global
rate limit of mDNS messages that can be created by a Web application, namely the
(optional) probing queries, announcing and goodbye responses, should be
provided by ICE implementations that implement this proposal.
...
would lead to the discarding of this host candidate as in Step 5 in Section
{{#gathering}}.
...
related to the gathered host candidates.
