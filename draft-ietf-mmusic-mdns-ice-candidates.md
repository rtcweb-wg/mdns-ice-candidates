...
1. Check whether this IP address satisfies the ICE agent's policy regarding
   ...
   from {{address-privacy}} below). If so, expose the candidate and abort
   this process.
...
#### Determining Address Privacy and Server-Reflexive Candidates {#address-privacy}
...
Regardless of whether the address turns out to be public or private, a
...
Because a new candidate is always generated by the STUN query, the ICE agent
...
To that end, once an address has been identified as public, the ICE agent MAY
...
information SHOULD be discarded, as the new set may return different results.
